...
def require_oauth(provider: str):
    """
    Decorator for agent tool functions that show UI widgets or perform actions requiring
    OAuth authentication. Checks for a valid token *before* the function body
    executes, ensuring no UI widget is rendered or action performed for an unauthenticated user.

    If the user is not authenticated, the OAuth challenge is returned immediately
    via ``context.oauth_start(provider)`` and registered in ``context.actions``.
    If authenticated, the wrapped function proceeds normally.

    Supports both ``async`` and synchronous tool functions safely.

    Usage::

        from app.core.hubscape_adk import require_oauth

        @require_oauth("mopl")
        async def show_my_form() -> dict:
            context = get_context()
            context.show_widget("my_widget")
            return {"status": "success", "message": "Widget shown."}
    """
    ...
    def decorator(func):
        ...
        if is_async:
            ...
            async def async_wrapper(*args, **kwargs):
                context = get_context()
                token = await context.get_oauth_token(provider)
                if not token:
                    res = context.oauth_start(provider)
                    if inspect.iscoroutine(res):
                        res = await res

                    system_action = res.get("system_action") if isinstance(res, dict) else None
                    if system_action and system_action not in context.actions:
                        context.actions.append(system_action)
            ...
            def sync_wrapper(*args, **kwargs):
                context = get_context()
                token = _safe_run_async(context.get_oauth_token(provider))
                if not token:
                    res = context.oauth_start(provider)
                    if inspect.iscoroutine(res):
                        res = _safe_run_async(res)
