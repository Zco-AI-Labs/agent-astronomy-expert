...
- [x] Update `app/SKILL.md` frontmatter and system prompt:
...
- [x] Configure `app/privileges.json`:
...
- [x] Confirm dependencies in `pyproject.toml` (verify `httpx` and standard math/datetime utilities are active).

---

### Phase 2: Core Astronomy & Meteorological Engines (`app/scripts/`)
- [x] Create `app/scripts/_weather_utils.py`:
...
- [x] Create `app/scripts/_ephemeris_utils.py`:
...
- [x] Implement `app/scripts/get_stargazing_forecast.py`:
...
- [x] Implement `app/scripts/get_celestial_body_position.py`:
...
- [x] Implement `app/scripts/get_astronomical_events.py`:
  - Retrieve active meteor showers (with peak rates and radiant directions), lunar phases, and conjunctions.
- [x] Implement `app/scripts/manage_stargazer_profile.py`:
...
- [x] Scaffold directory `app/ui/widgets/` if not present.
- [x] Create `app/ui/widgets/night_sky_forecast.json` using the complete Lego block schema from Section 7.
- [x] Create `app/ui/widgets/celestial_target_card.json` using the complete Lego block schema from Section 7.
- [x] Create `app/ui/widgets/stargazer_profile_card.json` using the complete Lego block schema from Section 7.

---

### Phase 4: Verification & Automated Testing
- [x] Create `tests/unit/test_ephemeris.py`:
...
- [x] Create `tests/unit/test_weather.py`:
  - Unit tests for Open-Meteo response parser with mock payloads.
- [x] Create `tests/integration/test_astronomy_tools.py`:
  - Integration tests verifying all 4 tools execute and return valid schema dictionaries with rendered widgets.
- [x] Run test suite: `uv run pytest tests/unit tests/integration -v`.
- [x] Run code quality checks: `ruff check .`.

---

### Phase 5: Interactive Simulation & Delivery
- [x] Verify interactive conversational flows in integration runner (`test_astronomy_dialogue.py`).
- [x] Test multi-modal fallbacks: SMS plain-text output and Voice natural phonetic phrasing.
- [x] Update `walkthrough.md` with test results and validation proof.
