...
> 3. The token placeholder in the headers of `app/config.json` (e.g., `${OAUTH_TOKEN:mopl}`).
...
### `@require_oauth(provider: str)`
A declarative function decorator for agent tools that enforce OAuth authentication before executing business logic or rendering frontend Lego widgets.

* **Pre-Execution Guard**: Verifies `context.get_oauth_token(provider)` before the tool function body executes.
* **Prevents Broken Widgets**: If unauthenticated, halts immediately, registers the `TRIGGER_OAUTH` action in `context.actions`, and returns `context.oauth_start(provider)` so no interactive widget is mounted without valid credentials.
* **Thread-Safe & Async/Sync**: Safely executes across asynchronous ADK runtimes and synchronous tool definitions without event loop conflicts.
* **Usage**:
  ```python
  from app.core.hubscape_adk import require_oauth, get_context

  @require_oauth("mopl")
  async def show_add_user_form() -> dict:
      context = get_context()
      context.show_widget("add_user_form")
      return {"status": "success", "message": "User form displayed."}
  ```
